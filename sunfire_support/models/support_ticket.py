...
class SupportTicket(models.Model):
    ...
    sub_tag_id=fields.Many2one('support.subtag',string="Sub Tag")
    sub_categ_id=fields.Many2one('support.subcategory',string="Sub category")
    ...
    def set_opf_name(self):
        ...
        for i in sale_order_obj:
            if i.state == 'sale' or i.state=='done':
                li.append(i.id)
        return [('id','in',li)]
    opf_name=fields.Many2one('sale.order',string='OPF Reference',domain=lambda self:self.set_opf_name())
    ...
    def set_subtag(self):
        ...
        sub_tags=[]
        if self.tag_id:
            for sub_tag in self.tag_id.tag_lines:
                sub_tags.append(sub_tag.id)
            domain['sub_tag_id']=[('id','in',sub_tags)]
    ...
    def set_subcategory(self):
        ...
        sub_categs=[]
        if self.category_id:
            for sub_categ in self.category_id.categ_lines:
                sub_categs.append(sub_categ.id)
            domain['sub_categ_id']=[('id','in',sub_categs)]
            return {'domain':domain}
...
class SupportCategory(models.Model):
    ...
    categ_lines = fields.One2many('support.subcategory','categ_id', string='Category Lines')
    
class SupportSubCategory(models.Model):
    _name="support.subcategory"
    _rec_name="name"
    _description="Sunfire Support Sub Category"
    name=fields.Char(string="Sub Category")
    categ_id = fields.Many2one('support.category', string='Category Id')
...
class SupportTag(models.Model):
    ...
    tag_lines = fields.One2many('support.subtag','tag_id', string='Sub Tag')
    
class SupportSubTag(models.Model):
    _name="support.subtag"
    _description="Sunfire Support Sub Tag"
    _rec_name="name"

    name=fields.Char(string="Sub Tag",help="The sub tag for the Tag")
    tag_id = fields.Many2one(comodel_name='support.tag', string='Tag Id')

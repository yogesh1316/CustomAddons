...
class purchase_line_inhe(models.Model):
    ...
    price_unit = fields.Float(string='Unit Price', required=False, digits=dp.get_precision('Product Price'),compute='set_price_unit',store=True)
    ...
    def set_price_unit(self):
        for order in self:
            if order.purchase_price:
                order.price_unit=order.purchase_price

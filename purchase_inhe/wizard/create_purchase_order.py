...
class PurchaseOrderLine(models.Model):
    ...
    # Ganesh      15/06/2018                
    # Info : Validation on pucrahse qty
    ...
    # create_by | create_date | update_by | update_date
    # Ganesh      15/06/2018                
    # Info : Update po_state on write function
    @api.model
    ...
    # create_by | create_date | update_by | update_date
    # Ganesh      15/06/2018                
    # Info : update sale order line item PO state NA witch is deleted from purchase order 
    @api.multi
    def unlink(self):
        sale_order_line_obj = self.env['sale.order.line']    
        if self.saleorder_line_id:
            for line in self:
...
class CreatePurchaseOrder(models.Model):
    ...
    # create_by | create_date | update_by | update_date
    # Ganesh      01/06/2018    Ganesh      20/09/18            
    # Info : This function use to create sale order to purchase order if state is done and po_state is 'NA'
    ...
    def call_purchase_order(self):
        ...
        # Update code get singal id  from partnerid
            for partnid in partnerid:
                partid = partnid[0].vendor_dr_id
                #print('partner IDS------',partid)
                break

            print('partnerid -------------',partid.id)
        
        partner_invoice_id = res_partner_obj.search([('parent_id','=',partid.id),('type','=','invoice')])
        print('partner_invoice_id 1-------------',len(partner_invoice_id))
        if len(partner_invoice_id) > 0:
            if partner_invoice_id[0]['id'] == False:
                partner_invoice_id = res_partner_obj.search([('id','=',partid.id),('type','=','contact')])
                print('partner_invoice_id 2-------------',partner_invoice_id[0]['id'])
        else:
            if partner_invoice_id.id == False:
                partner_invoice_id = res_partner_obj.search([('id','=',partid.id),('type','=','contact')])
                print('partner_invoice_id 3-------------',partner_invoice_id[0].id)
        ...
        #After creating purchase.order copy id to 'pur_order_dr_id' of uplaod_tab_info
        for i in sale_order_rec:
            if i.order_upload_line:
                for ids in i.order_upload_line:
                    #print("Purchase Order id======>",purchase_order_id)
                    ids.pur_order_dr_id=purchase_order_id.id

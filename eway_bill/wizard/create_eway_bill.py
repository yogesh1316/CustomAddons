...
class Createewaybill(models.Model):
    ...
    def set_vech_type(self,aidata):
       if aidata.vehicletype:
           #print("===============123",(aidata.vehicletype))
           return (aidata.vehicletype)  
       else: 
           return ''
    @api.multi
    def eway_bill_generate(self):
        ...
        head_list=''
        head_list=({
            "version":'1.0.1118', #Updated-By:Pradip,Updated-Date:14-Feb-2019 Info.Verson No. Updated
            "billLists":[]
        })
        ...
        for id in temp:
            ...
            #print('************',aidata.number,aidata.vehicletype)
            ...
            for hsncode in ail_data[:1]:
                    main_hsn_code=hsncode.product_id.l10n_in_hsn_code#Update-By:Pradip,Update-Date:08-Jan-2019
                    #print("main_HSN_CODE77777777777777777777",main_hsn_code)    
            data['billLists']=[]
            ...
            data['billLists'].append({
                    ...
                    "toAddr2": format(aidata.partner_id.street2) if aidata.partner_id.street2 else '' ,#Update-By:Pradip,Update-Date:08-Jan-2019
                    "toPlace": format(aidata.partner_id.city) if aidata.partner_id.city else '',#Update-By:Pradip,Update-Date:08-Jan-2019
                    ...
                    "transporterName":format(aidata.transportername) if aidata.transportername else '',#Update-By:Pradip,Update-Date:08-Jan-2019
                    "transporterId": format(aidata.transporterid) if aidata.transporterid else '' , #Update-By:Pradip,Update-Date:08-Jan-2019          
                    "transDocNo":format(aidata.transdocno) if aidata.transdocno else '', #Update-By:Pradip,Update-Date:08-Jan-2019
                    "transDocDate": format(transdocdate) ,
                    "vehicleNo":format(aidata.vehicleno) if aidata.vehicleno else '' ,#Update-By:Pradip,Update-Date:08-Jan-2019
                    "vehicleType": self.set_vech_type(aidata) ,#method2 #Update-By:Pradip,Update-Date:08-Jan-2019
                    #"vehicleType":format(aidata.vehicletype) if aidata.vehicletype else '' , method1
                    "totInvValue":aidata.amount_total,
                    "mainHsnCode": main_hsn_code,#Created-By:Pradip,Created-Date:08-Jan-2019
                    "transType":1,#Created-By:Pradip,Created-Date:08-Jan-2019
                    #"cessNonAdvol":0.0,#Created-By:Pradip,Created-Date:08-Jan-2019
                    "OthValue":0.0,#Created-By:Pradip,Created-Date:08-Jan-2019
                    "TotNonAdvolVal":0.0,#Created-By:Pradip,Created-Date:08-Jan-2019
                    "itemList": []
                })
            sgstamt=0
            cgstamt=0
            ...
            for n_list in data['billLists']:
                for line in ail_data:
                    ...
                    n_list['itemList'].append({
                                ...
                                "cessRate": 0,
                                "cessNonAdvol":0.0, #Created-By:Pradip,Created-Date:14-Feb-2019
                        })
                    i=i+1
                ##print("main_hnscode============999999999",int(line.product_id.l10n_in_hsn_code))
